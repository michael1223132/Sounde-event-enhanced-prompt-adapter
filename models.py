...
from reference_encoder.resnet_encoder import ResNetSpeakerEncoder, BaseAudioConfig,read_wav_file
...
class AudioDiffusion(nn.Module):
    def __init__(
        ...
        device='cuda',
        reference_dim = 1024,
        path = "/data4/xiongchenxu/tango/tango/pretrained/pytorch_model_main.bin"
    ):
        super().__init__()
        ...
        if unet_model_config_path:
            ...
            for key in pretrained_dict.keys():
                if key.startswith("unet."):
                    temp_key = key[len("unet."):]
                self.unet.state_dict()[temp_key] = pretrained_dict[key]
        ...
        self.reference_encoder = ResNetSpeakerEncoder
    ...
    def forward(self, latents, prompt, reference,prior_reg,validation_mode=False):
        device = self.text_encoder.device
        ...
        reference_model = self.reference_encoder(input_dim = 80, proj_dim = 1024, use_torch_spec= True, audio_config=audio_config).to(device)
        waveforms = [read_wav_file(file_path, target_length=16000).to(device) for file_path in reference]
        cross_attention = self.CrossAttention(n_heads=16, d_embed= encoder_hidden_states.size(-1), d_cross= self.reference_dim).to(device)
        waveforms = torch.stack(waveforms) 
        reference_embedding = reference_model(waveforms,l2_norm = True).to(device)

        reference_embedding = reference_embedding.unsqueeze(1)
        attn_score = cross_attention(encoder_hidden_states, reference_embedding)
        style_embedding = attn_score+encoder_hidden_states
        style_embedding = torch.mean(style_embedding, dim=1)
        ...
        if self.set_from == "random":
            #logging.info(f"random:n_l{noisy_latents},ts:{timesteps}")
            model_pred = self.unet(
                noisy_latents, timesteps, encoder_hidden_states, style_embedding,
            ...
            model_pred = self.unet(
                compressed_latents, timesteps, encoder_hidden_states, style_embedding,
                encoder_attention_mask=boolean_encoder_mask
            ).sample
    ...
    @torch.no_grad()
    def inference(self, prompt, reference,inference_scheduler,num_steps=20, guidance_scale=3, num_samples_per_prompt=1,
        ...
        if classifier_free_guidance:
            prompt_embeds, boolean_prompt_mask, style_reference= self.encode_text_refaudio_classifier_free(prompt, reference, num_samples_per_prompt)
        else:
        ...
        for i, t in enumerate(timesteps):
            ...
            reference_embedding = torch.cat([style_reference] * 2) if classifier_free_guidance else style_reference
            latent_model_input = inference_scheduler.scale_model_input(latent_model_input, t)

            noise_pred = self.unet(latent_model_input, t, encoder_hidden_states=prompt_embeds,class_labels = reference_embedding, encoder_attention_mask=boolean_prompt_mask).sample
    ...
    def encode_text_refaudio_classifier_free(self, prompt, reference, num_samples_per_prompt):
        device = self.text_encoder.device
        batch = self.tokenizer(
            prompt, max_length=self.tokenizer.model_max_length, padding=True, truncation=True, return_tensors="pt"
        )
        ...
        waveforms = [read_wav_file(file_path, target_length=16000).to(device) for file_path in reference]
        waveforms = torch.stack(waveforms) 
        cross_attention = self.CrossAttention(n_heads=16, d_embed= prompt_embeds.size(-1), d_cross= self.reference_dim).to(device)
        reference_embedding = reference_model(waveforms,l2_norm = True).to(device)
        reference_embedding = reference_embedding.unsqueeze(1)
        attn_score = cross_attention(prompt_embeds, reference_embedding)
        style_reference = attn_score+prompt_embeds
        style_embedding = torch.mean(style_reference, dim=1)

        #prompt_embeds = self.activation(self.linear(torch.concat((prompt_embeds,attn_score),dim=-1)))
        ...
        return prompt_embeds, boolean_prompt_mask,style_embedding